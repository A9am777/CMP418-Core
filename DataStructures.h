...
namespace Animation
{
  ...
  template<typename Data, typename Indexer = NamedHeapInfo> class NamedHeap
  {
    ...
    Data& add(Label name);
    NamedHeapInfo* getMetaInfo(Label name);
    const NamedHeapInfo* getMetaInfo(Label name) const;
    UInt getID(Label name) const;

    Data& get(UInt id) { return heapedData.at(id); }
    const Data& get(UInt id) const { return heapedData.at(id); }
    NamedHeapInfo* getMetaInfo(gef::StringId nameHash);
    const NamedHeapInfo* getMetaInfo(gef::StringId nameHash) const;
    UInt getID(gef::StringId nameHash) const;
    size_t getHeapSize() const { return heapedData.size(); }

    private:
    std::unordered_map<gef::StringId, NamedHeapInfo> metaMap;
  ...
  template<typename Data, typename Indexer>
  inline NamedHeapInfo* NamedHeap<Data, Indexer>::getMetaInfo(Label name)
  {
    return getMetaInfo(StringTable.Add(name));
  }
  template<typename Data, typename Indexer>
  inline const NamedHeapInfo* NamedHeap<Data, Indexer>::getMetaInfo(Label name) const
  {
    return getMetaInfo(StringTable.Add(name));
  }
  template<typename Data, typename Indexer>
  ...
  template<typename Data, typename Indexer>
  inline NamedHeapInfo* NamedHeap<Data, Indexer>::getMetaInfo(gef::StringId nameHash)
  {
    auto& metaIt = this->metaMap.find(nameHash);
    return metaIt == this->metaMap.end() ? nullptr : &metaIt->second;
  }
  template<typename Data, typename Indexer>
  inline const NamedHeapInfo* NamedHeap<Data, Indexer>::getMetaInfo(gef::StringId nameHash) const
  {
